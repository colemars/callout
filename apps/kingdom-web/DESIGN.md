...
- ~~**Stage 6a**~~ — SHIPPED (2026-08-05): Influence build-plots — each reserved plot has ONE designated monument in the spend catalog (fixed siting: the Wellspring on plot-b, the Founder's statue on plot-a, the Stargazers' Tower on plot-c); tapping a plot opens the in-canvas Masons' Yard panel (price vs. balance, drag-guarded Raise-it order through the existing CAS purchase path); built monuments reveal with a sparkle and survive as unlocks (staying behind on flee, like all cosmetics)
- ~~**Stage 6b**~~ — SHIPPED (2026-08-05): the real 50-frame atlas (gpt-image-1 via scripts/gen-art.mjs + pack-art.mjs) preloaded and aliased per manifest key, sketch prisms demoted to fallback
- **Stage 7** — the living camera: drag pan / pinch zoom / wheel zoom (Ctrl-⌘-gated inline, free when expanded), clamped to the map (cameraMath.ts pure, factor 1–2.5, factor 1 = the old fixed framing as the home pose); in-page expand overlay (viewport-filling, Esc collapses — NOT native fullscreen, which iPhones lack); DOM corner controls (expand + recenter-when-moved); the replay reel flies the camera to each located moment and home at the end, surrendering to any user gesture
